//Ritwika Das -- rd935
//g++ -std=c++17 -pthread hw3-2.cpp -o hw3-2 --> to compile
//./hw3-2 <directory_path> <num_threads> --> input
...
public:
    ...
    // Function to print the top 300 words by frequency
    void print_summary() const {
        // Copy word frequencies into a vector and sort them
        vector<pair<string, word_info>> word_list(dict.begin(), dict.end());

        // Sort by frequency in descending order
        sort(word_list.begin(), word_list.end(), [](const pair<string, word_info>& a, const pair<string, word_info>& b) {
            return a.second.count > b.second.count; // Sort by word count
        });

        // Print only the top 300 words
        cout << "Top 300 words by frequency:\n";
        int count = 0;
        for (const auto& entry : word_list) {
            if (count >= 300) break; // Stop after 300 words
...
int main(int argc, char* argv[]) {
    ...
    d.print_summary(); // Print the top 300 words
